...
def generate_score_report(
  ...
  """
    ...
    scorer = scorers.create_scorer_from_profile(score_type, case_insensitive=case_insensitive,
                                                meteor_directory=meteor_directory, options=options)
    ...
    if bootstrap != 0:
        ...
        for i in range(len(scores)):
            for j in range(i + 1, len(scores)):
                direcs.append((i, j))
        wins, sys_stats = sign_utils.eval_with_paired_bootstrap(ref, outs, src, scorer, direcs, num_samples=bootstrap,
...
def generate_word_accuracy_report(ref, outs,
  ...
  """
    ...
    if cache_dicts is None:
        statistics, my_ref_total_list, my_out_totals_list, my_out_matches_list = bucketer.calc_statistics(ref, outs,
    ...
    bucket_cnts, bucket_intervals = bucketer.calc_bucket_details(my_ref_total_list, my_out_totals_list,
                                                                 my_out_matches_list) if output_bucket_details else (
    None, None)
...
def generate_sentence_bucketed_report(ref, outs, src=None,
  ...
  """
    ...
    if ref_labels is not None:
        ...
        out_labels = [corpus_utils.load_tokens(out_label) if type(out_label) == str else out_label for out_label in
                      out_labels]
    ...
    bucketer = bucketers.create_sentence_bucketer_from_profile(bucket_type, bucket_cutoffs=bucket_cutoffs,
                                                               score_type=score_measure, label_set=label_set,
                                                               case_insensitive=case_insensitive)
    ...
    if cache_dicts is None:
        bcs = [bucketer.create_bucketed_corpus(out, ref=ref, src=src, ref_labels=ref_labels if ref_labels else None,
                                               out_labels=out_labels[i] if out_labels else None) for i, out in
               enumerate(outs)]
    ...
    if output_bucket_details and statistic_type == 'score':
        ...
        bucket_interval_calculator = lambda out, ref: \
        sign_utils.eval_with_paired_bootstrap(ref, [out], src, scorer, None)[1][0]
...
def generate_ngram_report(ref, outs,
  ...
  """
    ...
    if cache_dicts is None:
        ...
        ref_labels = corpus_utils.load_tokens(ref_labels) if type(ref_labels) == str else ref_labels
        out_labels = [corpus_utils.load_tokens(out_labels[i]) if not out_labels is None else None for i in
                      range(len(outs))]
        totals, matches, overs, unders = zip(
            *[ngram_utils.compare_ngrams(ref, out, ref_labels=ref_labels, out_labels=out_label,
                                         min_length=min_ngram_length, max_length=max_ngram_length) for out, out_label in
              zip(outs, out_labels)])
