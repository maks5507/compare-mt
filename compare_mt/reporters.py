...
class ScoreReport(Report):
    ...
    def plot(self, output_directory, output_fig_file, output_fig_format='pdf'):
        ...
        if self.wins:
            sys_errs = [np.array([[score - stat['lower_bound']], [stat['upper_bound'] - score]]) for (score, stat) in
                        zip(self.scores, self.sys_stats)]
        else:
            sys_errs = None
        xticklabels = None
                    ...
                    if self.ref_aligns:
                        _, _, _, src_buckets, ref_aligns, ref_matches = \
                            self.bucketer._calc_src_buckets_and_matches(self.src_sents[eid],
                                                                        self.src_labels[
                                                                            eid] if self.src_labels else None,
                                                                        self.ref_sents[eid],
                                                                        self.ref_aligns[eid],
                    ...
                    # Find buckets for the examples if it's on the target side
                        ...
                        _, _, _, ref_buckets, out_buckets, out_matches = \
                            self.bucketer._calc_trg_buckets_and_matches(self.ref_sents[eid],
                                                                        self.ref_labels[
                                                                            eid] if self.ref_labels else None,
                                                                        [x[eid] for x in self.out_sents],
                                                                        [x[eid] for x in
                                                                         self.out_labels] if self.out_labels else None)
                        ref_hls = [x == bi for x in ref_buckets]
                        out_hls = [[(b == bi and m >= 0) for (b, m) in zip(ob, om)] for (ob, om) in
                                   zip(out_buckets, out_matches)]
...
class NgramReport(Report):
    ...
    def print(self):
        ...
        for i, (left, right) in enumerate(self.compare_directions):
            ...
            for k, v in self.scorelist[i][:report_length]:
                print(
                    f"{' '.join(k)}\t{fmt(v)} (sys{left + 1}={self.matches[left][k]}, sys{right + 1}={self.matches[right][k]})")
            print()
    ...
    def html_content(self, output_directory=None):
        ...
        for i, (left, right) in enumerate(self.compare_directions):
            ...
            table.extend([[' '.join(k), fmt(v), self.matches[left][k], self.matches[right][k]] for k, v in
                          self.scorelist[i][:report_length]])
            html += html_table(table, title)

            title = f'{report_length} n-grams where {sys_names[right]}>{sys_names[left]} in {self.compare_type}'
            table = [['n-gram', self.compare_type, f'{sys_names[left]}', f'{sys_names[right]}']]
            table.extend([[' '.join(k), fmt(v), self.matches[left][k], self.matches[right][k]] for k, v in
                          reversed(self.scorelist[i][-report_length:])])
            html += html_table(table, title)
        return html
...
class SentenceReport(Report):
    ...
    def __init__(self, bucketer=None, sys_stats=None, statistic_type=None, scorer=None, bucket_cnts=None,
                 bucket_intervals=None, title=None):
        self.bucketer = bucketer
...
class SentenceExampleReport(Report):
    ...
    def __init__(self, report_length=None, scorediff_lists=None, scorer=None, ref=None, outs=None, src=None,
                 compare_directions=[(0, 1)], title=None):
        self.report_length = report_length
...
def tag_str(tag, str, new_line=''):
    return f'<{tag} style="text-align:left;">{new_line} {str} {new_line}</{tag}>'
